# 👋 Hi, I'm Atte Kivimäki

### Software Developer
### Full Stack Development • AI Workflows • Game Audio

I'm a software developer with a background in music.

My main project is **AJ-OS** — a personal operating system that maintains an evolving working context by turning everyday work into durable knowledge.

Alongside software engineering, I'm building a technical game audio portfolio focused on interactive audio implementation with Unity and Wwise.

---

## 🚀 Currently Building

### ⭐ AJ-OS

A platform that connects existing tools through shared context instead of replacing them.

Core loop:

> **Work → Durable Knowledge → Maintained Context → Better Work**

Current focus:

- Knowledge systems
- Software architecture
- AI-assisted workflows
- Developer tooling
- Documentation platforms
- Automation

### 🎮 Unity + Wwise Fantasy Audio Demo

A technical game audio portfolio demonstrating:

- Interactive music
- Gameplay audio
- Environmental acoustics
- Unity ↔ Wwise integration
- Scalable audio architecture
- Original music & sound design

---

## 🛠 Technologies

**Software**

TypeScript • JavaScript • Node.js • React • Express • Prisma • SQL • Docker • AWS • GitHub Actions

**AI & Automation**

Claude SDK • OpenAI APIs • n8n

**Game Audio**

Unity 6 • Wwise 2025 • C#

---

## 🌟 Other Projects

- 🟣 **J. Partynen** — Artist website & custom CMS
- 🔵 **Au2mate** — Vehicle marketplace backend

---

## 📫 Contact

[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:aj.kivimaki@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/aj-kivimaki/)
